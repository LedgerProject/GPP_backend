...
export class DocumentController {
  ...
  private async saveDocumentChunk(objectToSave: any, documentUUIDReference: string): Promise<DocumentEncryptedChunk> {
    ...
    const savedChunk = await this.documentEncryptedChunkRepository.save(documentsEncryptedChunk);

    documentsEncryptedChunk.transactionId = await writeIntoBlockchain(jsonToSave, savedChunk.getId());
